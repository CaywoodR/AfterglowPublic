// Includes base sum and x-ray integral on the basis of a broken power law assumption
#include <iostream>
#include <fstream>
#include <cmath>
#include <iomanip>
...
double sum_fx = 0.0;
double total_fx = 0.0;
double h_max = 1.0; // max theta
double p_max = 0.0; // max phi
...
double G(double x) // Initial Gamma: takes in an angle in radians
{
    if (x < 0.1) {
        ...
        return 1.0 + (G1 - 1.0) * pow(x / t1, -4.5);
    }
}
   ...
   // int end_patch = fx.size();
    ...
    for(int i=1; i<steps; i++)//change deltat for log
    {
		...
		ppxp.at(i) = p_p_x_p(Bpoft.at(i),goft.at(i),doft.at(i));
		  // }
		//}
		//double reciprocal_max = 1.0 / std::max({1.5E-28, 4 * M_PI * pow(D, 2.0) * n * (pow(roft.at(i), 3.0) / 3.0), pow(doft.at(i), 3), ppnup.at(i), sin(rdeg(z)), rdeg(z), rdeg(x)});
		fxr.at(i) = (1/1.50E-20)*(1 / (4*M_PI*pow(D,2.0))*n*(pow(roft.at(i), 3.0) / 3.0)) * pow(doft.at(i),3)*ppxp.at(i)*sin(rdeg(z))*rdeg(z)*rdeg(x);//theta-phi
        fx.at(i) = (1/1.5E-28)*(1 / (4*M_PI*pow(D,2.0))*n*(pow(roft.at(i), 3.0) / 3.0)) * pow(doft.at(i),3)*ppnup.at(i)*sin(rdeg(z))*rdeg(z)*rdeg(x);//theta-phi
		sum_fx[i] += fx.at(i);
		sum_fxr[i] += fxr.at(i);
	...
	ofstream last_entries("sum_fx.txt");
