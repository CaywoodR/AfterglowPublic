...
def mkmov_simple(starti=0,endi=400,length=210):
    for i in np.arange(starti,endi+1):
        rd("dump%03d" % i);
        aphi=psicalc()
        if i == starti: amax = aphi.max()
        cs, cb = plco(np.log10(rho),levels=np.linspace(-8,0,100),isfilled=1,k=0,xy=1,xmax=100,ymax=10,dobh=1,cb=1,extend="both",pretty=1)
        ax = plt.gca()
        ...
        plt.xlim(0,length/2);plt.ylim(-0.5*length,0.5*length)
        plt.draw();
...
def mkmov(startn=0,endn=-1,ln=100,whichi=0,whichn=1,**kwargs):
...
if __name__ == "__main__":
    ...
    if False:
        ...
        plc(r,np.log10(),cb=True,xy=1,xmax=100,ymax=50)
...
#I modified the script from here, the wrapper for the movie maker is on line 1123 whilst the extraction begins on line 1150. 
if __name__ == "__main__":
    if len(sys.argv)>1:
        ...
        if True:
            ...
            rd("dump180")
            #R-z plot of the logarithm of density distribution
            #plc(np.log10(gamma),cb=True,xy=1,xmax=100,ymax=50)
             
            
            #B-Flux and plot field lines
            psi = psicalc()
            ...
            gamma = alpha*uu[0]
            #plc(np.log10(gamma),cb=True,xy=1,xmax=100,ymax=50)
            #Energy output of Black Hole
            aux() #compute OmegaF and T^\mu_\nu
            dEr = -gdet*Tud[1,0]*-rho*uu[1]*_dx2*_dx3
            Er = dEr.sum(1) #in phi (axis=2) and theta (axis=1)
            #print(Er) WHY
            #Total Radial Energy vs. Flux
            #x=r[:,0,0] #x is now the 1D radial coordinate
            theta = h[-iofr(rx), :, 0] * 180 / np.pi
            #print(-Er)
            plt.plot(theta,-Er,label = r"E_tot", lw = 4, color = 'green') #make a 1D plot of energy ﬂux vs. x 0.00247061
            plt.yscale("log") #logarithmic x-axis
            plt.ylim(1/100,10)
            plt.xlim(0,90) #limit x-range to 20r_g
            plt.xlabel(r"$\theta$ [$^{\circ}$]", fontsize=20)
            #plt.legend(loc = "best")
            plt.title(r"$r=$%g $r_0$, t=%g, %gx%gx%g" %(rx,t,nx,ny,nz), fontsize=20)
            fig = plt.figure()
            ax = fig.add_subplot(111)
            a=np.max(Er[iofr(rx),0:ny])
            m, = plt.plot(h[iofr(rx),:,0]*180/3.14,(1.0/.00247061)*Er.reshape(-1),label=r"$\dot{E}_{TOT}$",lw=4, alpha=2, color='green')
            
            k, = plt.plot(h[iofr(rx),:,0]*180/np.pi,gamma[iofr(rx),:,0].reshape(-1),label=r"$\Gamma$",lw=4, alpha=4, color='orange')
          
            plt.yscale('log')
            plt.xlim(0,90)
            plt.ylim(1/10000,100)
            plt.xlabel(r"$\theta$ [$^{\circ}$]", fontsize=20)
            plt.legend(loc="best")              
            plt.title(r"$r=$%g $r_0$, t=%g, %gx%gx%g" %(rx,t,nx,ny,nz), fontsize=20)
            ...
            #data sets
            h_data = h[-iofr(rx), :, 0] * 180 / np.pi
            Er_data = -Er
            gamma_data = gamma[iofr(rx), :, 0].reshape(-1)
            
            #Write to file extract.txt
            file = "extract.txt"
            with open(file, 'w') as file:
                file.write("h\t-Er\tgamma[iofr(rx),:,0]\n")
                for theta, Er, gamma in zip(h_data, Er_data, gamma_data):
                    file.write(f"{theta}\t{(1/0.0247061)*Er}\t{gamma}\n") #0.00251766

            #print("Finished") Clean this up 
            
            #Field line omega vs. Theta on Horizon
            ...
            #np.savetxt("etheta.txt", data, header="x Er", fmt="%.6f")
            #Red Ergoline
            rergo = 1+np.sqrt(1-(a*np.cos(h))**2)
            plc(r-rergo,levels=(0,),xy=1,colors="red",linewidths=2)
            
            #modified PDF for Lorentz density - try understanding this plot later 
            #Show Black Hole element as Black Circle
            ...
            plt.figure()
            #Make Movie
            mkmov_simple(starti = 0,endi = 500)